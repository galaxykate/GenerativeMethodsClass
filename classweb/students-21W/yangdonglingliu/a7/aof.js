...
class AoF  {
	constructor(labels) {
		...
		for (var i = 0; i < labels.length; i++) {
			this.values[i] = Math.floor(Math.random()*10)
		}
		...
		for (var i = 0; i < this.values.length; i++) {
			...
			switch(this.mode) {
				...
				case "wander":
					...
					Vue.set(this.values, i, Math.floor(noise(i*50 + 190*this.idNumber, t)*10))
			}
	...
	cloneParent(aof, mutationValue=.1) {
		for (var i = 0; i < this.values.length; i++) {
			...
			Vue.set(this.values, i, Math.floor(val*10))
		}
		return this
	}
	...
	randomize() {
		for (var i = 0; i< this.values.length; i++) {
			Vue.set(this.values, i, Math.floor(Math.random()*10))
		}
	}
		...
		<table>
			<tr v-for="(value,valIndex in aof.values">
				...
				<td class="slider-cell">
					...
					<input type="range" min="0" max="9" :step="1" class="slider" :value="value" @input="ev => change(ev, valIndex)" />
				</td>
