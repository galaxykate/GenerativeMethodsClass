...
let app = {
	...
	// classes: [Face,LTree],
	currentClass: CirclePattern,
	xaxis: CirclePattern.labels[0],
	yaxis: CirclePattern.labels[1],
	...
	populate() {
		...
		for (var i = 0; i < app.popCount; i++) {
			...
			// let pct = app.popCount==1?0:i/(app.popCount - 1)
			let pct = app.popCount==1?.5:i/(app.popCount - 1)
			let x = canvasW*.5 + (canvasW - 100)*.8*(pct - .5)
			let y = canvasH*(.7 - .4*(i%2))
...
document.addEventListener("DOMContentLoaded", function(){
	...
	new Vue({
		...
		methods: {
			...
			reroll() {
				Vue.set(app, "selected" , app.hovered)
				app.populate()
			...
			let p = new p5((p) => {
				...
				p.draw = () => {
					...
					if (app.showGraph) {
						...
						p.translate(0, 0)
						p.fill(100)
						p.rect(0, 0, graphSize+5, graphSize+5)

						app.individuals.forEach(ind => {
							let x = ind.aof.get(app.xaxis)/10
							let y = 1 - ind.aof.get(app.yaxis)/10
							p.fill(...ind.aof.idColor)
							p.circle(x*graphSize, y*graphSize, 5)
						})
